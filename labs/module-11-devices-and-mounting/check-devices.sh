...
# 6. Evidence of a successful MOUNT of the practice disk. df -hT for a mounted
#    loop device shows a line containing the mount point 'practicedisk' along
#    with a size/use column. Require 'practicedisk' to appear in the report.
if grep -q 'practicedisk' "$REPORT"; then
  ok "report shows evidence of mounting the practice disk (df/mount output references /mnt/practicedisk)"
else
  no "report has no mount evidence for /mnt/practicedisk — while it's mounted, run:  df -hT | grep practicedisk >> ~/module11-devices-report.txt"
fi

# 7. Evidence that fsck was run on the loop device. fsck/e2fsck output contains
#    recognizable phrases ('clean', 'Pass 1', 'e2fsck', or the version banner).
if grep -Eqi 'fsck|e2fsck|Pass 1|Pass 2|is clean|blocks' "$REPORT"; then
  ok "report shows fsck was run on the loop device"
else
  no "report has no fsck evidence — run 'sudo fsck -f /dev/loopX' on the UNMOUNTED disk and append its output"
fi
...
if (( fstab_ok == 1 )); then
  ...
  if [[ -z "$fstab_line" ]]; then
    no "no /etc/fstab line mounts /mnt/practicedisk — add:  /home/ubuntu/loopdisk.img  /mnt/practicedisk  ext4  loop,noauto  0  2"
  else
    no "your /etc/fstab line for /mnt/practicedisk is malformed — it needs source, mount point, type, and options (use the file+'loop' form or UUID= form)"
  fi
fi
