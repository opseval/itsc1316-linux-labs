...
- **Line endings on Windows.** If you edit shell scripts in a Windows editor that adds CRLF line endings, they may misbehave inside the Linux VM. Use VS Code (it defaults to LF for `.sh` files) or set `git config --global core.autocrlf input` once.
